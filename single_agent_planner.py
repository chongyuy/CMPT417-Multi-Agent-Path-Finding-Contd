...
def build_constraint_table(constraints, agent):
    ...
    for constraint in constraints:
        ...
        if constraint['agent'] == agent:
            ...
            if timestep not in table:
                table[timestep] = []
            table[timestep].append(new_constraint)
    return table
...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if next_time in constraint_table:
        item_list = constraint_table[next_time]
        for item in item_list:
            loc = item['loc']
            if item['positive']:
                if len(loc) == 1:
                    if item['loc'][0] != next_loc:
                        return True
                else:
                    if item['loc'][0] != curr_loc or item['loc'][1] != next_loc:
                        return True
            else:
                if (len(loc) == 1 and loc[0] == next_loc) or (len(loc) == 2 and loc[0] == curr_loc and loc[1] == next_loc):
                    return True
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    root = {'loc': start_loc, 'g_val': 0, 'h_val': h_value, 't_step': 0, 'parent': None}
    ...
    while len(open_list) > 0:
        ...
        if curr['loc'] == goal_loc:
            if len(constraints_table) == 0:
                ...
                max_index = max(constraints_table)
                curr_timestep = curr['t_step']

                time_count = max_index
                while curr_timestep < time_count:
                    if is_constrained(goal_loc, goal_loc, time_count, constraints_table):
                        break
                    time_count -= 1
        ...
        for dir in range(5):
            ...
            next_timestep = curr['t_step'] + 1
            if child_loc[0] < 0 or child_loc[0] >= len(my_map) or child_loc[1] < 0 or child_loc[1] >= len(my_map[0]):
                continue
            if my_map[child_loc[0]][child_loc[1]] or is_constrained(curr['loc'], child_loc, next_timestep,
                                                                    constraints_table):
                continue
            child = {'loc': child_loc,
                     'g_val': curr['g_val'] + 1,
                     'h_val': h_values[child_loc],
                     't_step': next_timestep,
                     'parent': curr}
